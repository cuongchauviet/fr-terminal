 ...
 */
@RestController
@RequestMapping("api/device-ip")
public class DeviceIpController extends GeneralMessage {
	...
	public ResponseEntity<ResponseResult> getAll(){
		...
		try {
			List<DeviceIp> deviceIp = deviceIpService.getAll();
			if(!deviceIp.isEmpty()) {
				responseResult.setSuccess(true);
				responseResult.setMessage(getDataSuccess);
				responseResult.setData(deviceIp);
			} else {
	...
	public ResponseEntity<ResponseResult> getById(@PathVariable("id") int id){
		ResponseResult responseResult = new ResponseResult();
		try {
			DeviceIp deviceIp = deviceIpService.getById(id);
			if(deviceIp != null) {
				responseResult.setSuccess(true);
				responseResult.setMessage(getDataSuccess);
				responseResult.setData(deviceIp);
	...
	public ResponseEntity<ResponseResult> update(@RequestBody @Valid DeviceIpDTO deviceIpDTO, BindingResult bindingResult){
		...
		try {
			if(!bindingResult.hasErrors()) {
				if(deviceIpService.update(deviceIpDTO)) {
					responseResult.setSuccess(true);
	...
	public ResponseEntity<ResponseResult> insert(@RequestBody @Valid DeviceIpDTO deviceIpDTO, BindingResult bindingResult){
		...
		try {
			if(!bindingResult.hasErrors()) {
				if(deviceIpService.insert(deviceIpDTO)) {
					responseResult.setSuccess(true);
