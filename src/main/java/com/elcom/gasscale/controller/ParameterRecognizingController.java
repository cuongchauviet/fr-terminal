 ...
 */
@RestController
@RequestMapping("api/parameter-recognizing")
@CrossOrigin
public class ParameterRecognizingController extends GeneralMessage {
	
	private final ParameterRecognizingService parameterRecognizingService;
	
	@Autowired
	public ParameterRecognizingController(ParameterRecognizingService parameterRecognizingService) {
		this.parameterRecognizingService = parameterRecognizingService;
	}
	...
	public ResponseEntity<ResponseResult> getAll(){
		...
		try {
			List<ParameterRecognizing> users = parameterRecognizingService.getAll();
			if(!users.isEmpty()) {
	...
	public ResponseEntity<ResponseResult> getById(@PathVariable("id") int id){
		ResponseResult responseResult = new ResponseResult();
		try {
			ParameterRecognizing user = parameterRecognizingService.getById(id);
			if(user != null ) {
				responseResult.setSuccess(true);
				responseResult.setMessage(getDataSuccess);
	...
	public ResponseEntity<ResponseResult> update(@RequestBody @Valid ParameterRecognizingDTO parameterRecognizingDTO, BindingResult bindingResult){
		ResponseResult responseResult = new ResponseResult();
		try {
			if(!bindingResult.hasErrors()) {
				if(parameterRecognizingService.update(parameterRecognizingDTO)) {
					responseResult.setSuccess(true);
	...
	public ResponseEntity<ResponseResult> insert(@RequestBody @Valid ParameterRecognizingDTO parameterRecognizingDTO, BindingResult bindingResult){
		ResponseResult responseResult = new ResponseResult();
		try {
			if(!bindingResult.hasErrors()) {
				if(parameterRecognizingService.insert(parameterRecognizingDTO)) {
					responseResult.setSuccess(true);
	...
	public ResponseEntity<ResponseResult> delete(@PathVariable("id") int id){
		ResponseResult responseResult = new ResponseResult();
		try {
			if(parameterRecognizingService.delete(id)) {
				responseResult.setSuccess(true);
